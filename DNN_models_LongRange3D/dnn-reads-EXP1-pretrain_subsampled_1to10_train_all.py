...
X_train_normalized_subsampled, y_train_subsampled = subsample_data(X_train_normalized, y_train, imbalance_ratio=10)
X_valid_normalized_subsampled, y_valid_subsampled = subsample_data(X_valid_normalized, y_valid, imbalance_ratio=10)
X_test_normalized_subsampled, y_test_subsampled = subsample_data(X_test_normalized, y_test, imbalance_ratio=10)
...
print "Performance on Valid: ", (dnn.test(X_valid_normalized[:, :, :11, :], y_valid))
print "Performance on Train: ", (dnn.test(X_train_normalized[:, :, :11, :], y_train))
...
np.save("model_predictions/test_set_upperTriangle_noDiag_reads_with_distances_pretr_subs_1to10_train_all.npy", pred_probs_test)
pred_probs_valid = dnn.predict(X_valid_normalized[:, :, :11, :])
np.save("model_predictions/valid_set_upperTriangle_noDiag_reads_with_distances_pretr_subs_1to10_train_all.npy", pred_probs_valid)
pred_probs_train = dnn.predict(X_train_normalized[:, :, :11, :])
np.save("model_predictions/train_set_upperTriangle_noDiag_reads_with_distances_pretr_subs_1to10_train_all.npy", pred_probs_train)
